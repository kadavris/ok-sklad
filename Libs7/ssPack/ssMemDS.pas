...
uses 
  Windows, SysUtils, Classes, Controls, DB, Variants;
...
{ TMemoryRecord }
  ...
  private
    FMemoryData: TssMemoryData;
    FID: Integer;
    FData: Pointer;
    FBlobs: Pointer;
    function GetIndex: Integer;
    procedure SetMemoryData(Value: TssMemoryData; UpdateParent: Boolean);
  protected
    procedure SetIndex(Value: Integer); virtual;
  public
    constructor Create(MemoryData: TssMemoryData); virtual;
    constructor CreateEx(MemoryData: TssMemoryData; UpdateParent: Boolean); virtual;
    destructor Destroy; override;
    property MemoryData: TssMemoryData read FMemoryData;
...
const
  ...
  ftSupported = [ftString, ftSmallint, ftInteger, ftWord, ftBoolean, ftFloat,
    ...
    ftLargeint, ftVariant, ftGuid, ftFMTBCD]
    + ftBlobTypes;
...
procedure AssignRecord(Source, Dest: TDataSet; ByName: Boolean);
  var
    ...
    F, FSrc: TField;
begin
  if not (Dest.State in dsEditModes) then DatabaseError(SNotEditing);
...
var
  ...
  begin
    if Field.DataType = ftString then begin
      ...
      if (loPartialKey in Options) then Delete(S, Length(Value) + 1, MaxInt);

      if (loCaseInsensitive in Options)
        then Result := AnsiCompareText(S, Value) = 0
        else Result := AnsiCompareStr(S, Value) = 0;
    end
  ...
  begin
    if FieldCount = 1
    then Result := CompareField(TField(Fields.First), KeyValues)
    else begin
      Result := True;
  ...
  try
    ...
    try
      ...
      try
        ...
        if not Result and DataSet.BookmarkValid(PChar(Bookmark))
          then DataSet.Bookmark := Bookmark;
      end;
...
begin
  ...
  case FieldType of
    ftString:
      if CaseInsensitive
        then Result := AnsiCompareText(PChar(Data1), PChar(Data2))
        else Result := AnsiCompareStr(PChar(Data1), PChar(Data2));

    ftSmallint:
      if SmallInt(Data1^) > SmallInt(Data2^)
      then Result := 1
      else if SmallInt(Data1^) < SmallInt(Data2^) then Result := -1;

    ftInteger, ftDate, ftTime, ftAutoInc:
      if Longint(Data1^) > Longint(Data2^)
      then Result := 1
      else if Longint(Data1^) < Longint(Data2^) then Result := -1;

    ftWord:
      if Word(Data1^) > Word(Data2^)
      then Result := 1
      else if Word(Data1^) < Word(Data2^) then Result := -1;

    ftBoolean:
      if WordBool(Data1^) and not WordBool(Data2^)
      then Result := 1
    ...
    ftDateTime:
      if TDateTime(Data1^) > TDateTime(Data2^)
      then Result := 1
    ...
    ftWideString:
      if CaseInsensitive
        then Result := AnsiCompareText(WideCharToString(PWideChar(Data1)),
                  WideCharToString(PWideChar(Data2)))
        else Result := AnsiCompareStr(WideCharToString(PWideChar(Data1)),
                  WideCharToString(PWideChar(Data2)));
    ftLargeint:
...
begin
  if not (FieldType in ftSupported)
    then Result := 0
  else if (FieldType in ftBlobTypes)
    then Result := SizeOf(Longint)
  else begin
    Result := Size;
    case FieldType of
      ftString: Inc(Result);
      ...
      ftBCD: Result := SizeOf(TBCD); //34;
      ftFMTBCD: Result := SizeOf(TBCD); //34;
...
//==============================================================================================
//==============================================================================================
constructor TMemoryRecord.Create(MemoryData: TssMemoryData);
begin
  CreateEx(MemoryData, True);
end;

//==============================================================================================
constructor TMemoryRecord.CreateEx(MemoryData: TssMemoryData; UpdateParent: Boolean);
begin
  inherited Create;
...
begin
  if FMemoryData <> nil
    then Result := FMemoryData.FRecords.IndexOf(Self)
    else Result := -1;
end;

//==============================================================================================
procedure TMemoryRecord.SetMemoryData(Value: TssMemoryData; UpdateParent: Boolean);
  var
    I: Integer;
    DataSize: Integer;
begin
...
//==============================================================================================
constructor TssMemoryData.Create(AOwner: TComponent);
begin
...
//==============================================================================================
destructor TssMemoryData.Destroy;
begin
  inherited Destroy;
  FreeIndexList;
  ClearRecords;
...
//==============================================================================================
function TssMemoryData.GetCapacity: Integer;
begin
...
//==============================================================================================
procedure TssMemoryData.SetCapacity(Value: Integer);
begin
  if FRecords <> nil then FRecords.Capacity := Value;
end;

//==============================================================================================
function TssMemoryData.AddRecord: TMemoryRecord;
begin
  Result := TMemoryRecord.Create(Self);
end;

//==============================================================================================
function TssMemoryData.FindRecordID(ID: Integer): TMemoryRecord;
var
  I: Integer;
begin
...
//==============================================================================================
function TssMemoryData.InsertRecord(Index: Integer): TMemoryRecord;
begin
  Result := AddRecord;
  Result.Index := Index;
end;
...
//==============================================================================================
function TssMemoryData.GetMemoryRecord(Index: Integer): TMemoryRecord;
begin
...
{ Field Management }
//==============================================================================================
// this is a copy from DB.pas with exception that not only fkData fields being processed
procedure TssMemoryData.CreateFieldDefs(Fields: TFields; FieldDefs: TFieldDefs);
  var
    I: Integer;
    F: TField;
    FieldDef: TFieldDef;
begin
  ...
  begin
    ...
    with F do begin
      ...
      //if Required then FieldDef.Attributes := [faRequired];
      //if ReadOnly then FieldDef.Attributes := FieldDef.Attributes + [faReadonly];

      if (DataType = ftBCD) and (F is TBCDField)
        then FieldDef.Precision := TBCDField(F).Precision
        else if (DataType = ftFMTBcd) and (F is TFMTBCDField) then FieldDef.Precision := TFMTBCDField(F).Precision;

      if F is TObjectField then CreateFieldDefs(TObjectField(F).Fields, FieldDef.ChildDefs);
    end;
  end;
end;

//==============================================================================================
procedure TssMemoryData.InitFieldDefsFromFields;
  var
    I: Integer;
    Offset: Word;
begin
  if FieldDefs.Count = 0 then begin
    for I := 0 to FieldCount - 1 do
      with Fields[I] do begin
        if FieldKind = fkInternalCalc then FieldKind := fkData;
        
        if (FieldKind in fkStoredFields) and not (DataType in ftSupported)
          then ErrorFmt(SUnknownFieldType, [DisplayName]);
      end;
  ...
  // we using now our own copy of the std routine to re-create all field types instead of fkData only
...
//==============================================================================================
function TssMemoryData.FindFieldData(Buffer: Pointer; Field: TField): Pointer;
  var
    Index: Integer;
begin
...
//==============================================================================================
function TssMemoryData.CalcRecordSize: Integer;
  var
    I: Integer;
begin
  Result := 0;
...
//==============================================================================================
procedure TssMemoryData.InitBufferPointers(GetProps: Boolean);
begin
...
//==============================================================================================
procedure TssMemoryData.ClearRecords;
begin
...
//==============================================================================================
function TssMemoryData.AllocRecordBuffer: PChar;
begin
...
//==============================================================================================
procedure TssMemoryData.FreeRecordBuffer(var Buffer: PChar);
begin
  if BlobFieldCount > 0 then
...
//==============================================================================================
procedure TssMemoryData.ClearCalcFields(Buffer: PChar);
begin
  FillChar(Buffer[FRecordSize], CalcFieldsSize, 0);
end;

//==============================================================================================
procedure TssMemoryData.InternalInitRecord(Buffer: PChar);
  var
    I: Integer;
begin
...
//==============================================================================================
procedure TssMemoryData.InitRecord(Buffer: PChar);
begin
...
//==============================================================================================
function TssMemoryData.GetCurrentRecord(Buffer: PChar): Boolean;
begin
  Result := False;
...
//==============================================================================================
procedure TssMemoryData.RecordToBuffer(Rec: TMemoryRecord; Buffer: PChar);
  var
    I: Integer;
begin
...
//==============================================================================================
function TssMemoryData.GetRecord(Buffer: PChar; GetMode: TGetMode; DoCheck: Boolean): TGetResult;
  var
    Accept: Boolean;
begin
  Result := grOk;
  ...
  if Result = grOk 
    then RecordToBuffer(Records[FRecordPos], Buffer)
    else if (Result = grError) and DoCheck then Error(SMemNoRecords);
end;

//==============================================================================================
function TssMemoryData.GetRecordSize: Word;
begin
  Result := FRecordSize;
end;

//==============================================================================================
function TssMemoryData.GetActiveRecBuf(var RecBuf: PChar): Boolean;
begin
  case State of
    dsBrowse:
...
//==============================================================================================
function TssMemoryData.GetFieldData(Field: TField; Buffer: Pointer): Boolean;
  var
    RecBuf, Data: PChar;
    VarData: Variant;
begin
...
//==============================================================================================
procedure TssMemoryData.SetFieldData(Field: TField; Buffer: Pointer);
  var
    RecBuf, Data: PChar;
    VarData: Variant;
begin
...
//==============================================================================================
procedure TssMemoryData.SetFiltered(Value: Boolean);
begin
...
//==============================================================================================
procedure TssMemoryData.SetOnFilterRecord(const Value: TFilterRecordEvent);
begin
  if Active then begin
    CheckBrowseMode;
...
//==============================================================================================
function TssMemoryData.RecordFilter: Boolean;
  var
...
//==============================================================================================
function TssMemoryData.GetBlobData(Field: TField; Buffer: PChar): TMemBlobData;
begin
  Result := PMemBlobArray(Buffer + FBlobOfs)[Field.Offset];
end;

//==============================================================================================
procedure TssMemoryData.SetBlobData(Field: TField; Buffer: PChar; Value: TMemBlobData);
begin
...
//==============================================================================================
procedure TssMemoryData.CloseBlob(Field: TField);
begin
  if (FRecordPos >= 0) and (FRecordPos < FRecords.Count) and (State = dsEdit)
  then PMemBlobArray(ActiveBuffer + FBlobOfs)[Field.Offset] :=
      PMemBlobArray(Records[FRecordPos].FBlobs)[Field.Offset]
  else PMemBlobArray(ActiveBuffer + FBlobOfs)[Field.Offset] := '';
end;

//==============================================================================================
function TssMemoryData.CreateBlobStream(Field: TField; Mode: TBlobStreamMode): TStream;
begin
...
{ Bookmarks }
//==============================================================================================
function TssMemoryData.BookmarkValid(Bookmark: TBookmark): Boolean;
begin
  Result := FActive and (TBookmarkData(Bookmark^) > Low(Integer)) and (TBookmarkData(Bookmark^) <= FLastID);
end;

//==============================================================================================
function TssMemoryData.CompareBookmarks(Bookmark1, Bookmark2: TBookmark): Integer;
begin
  if (Bookmark1 = nil) and (Bookmark2 = nil) then Result := 0
  else if (Bookmark1 <> nil) and (Bookmark2 = nil) then Result := 1
  else if (Bookmark1 = nil) and (Bookmark2 <> nil) then Result := -1
  else if TBookmarkData(Bookmark1^) > TBookmarkData(Bookmark2^)
       then Result := 1
  else if TBookmarkData(Bookmark1^) < TBookmarkData(Bookmark2^)
       then Result := -1
  else Result := 0;
...
//==============================================================================================
procedure TssMemoryData.GetBookmarkData(Buffer: PChar; Data: Pointer);
begin
  Move(PMemBookmarkInfo(Buffer + FBookmarkOfs)^.BookmarkData, Data^, SizeOf(TBookmarkData));
end;

//==============================================================================================
procedure TssMemoryData.SetBookmarkData(Buffer: PChar; Data: Pointer);
begin
  Move(Data^, PMemBookmarkInfo(Buffer + FBookmarkOfs)^.BookmarkData, SizeOf(TBookmarkData));
end;

//==============================================================================================
function TssMemoryData.GetBookmarkFlag(Buffer: PChar): TBookmarkFlag;
begin
  Result := PMemBookmarkInfo(Buffer + FBookmarkOfs)^.BookmarkFlag;
end;

//==============================================================================================
procedure TssMemoryData.SetBookmarkFlag(Buffer: PChar; Value: TBookmarkFlag);
begin
  PMemBookmarkInfo(Buffer + FBookmarkOfs)^.BookmarkFlag := Value;
end;

//==============================================================================================
procedure TssMemoryData.InternalGotoBookmark(Bookmark: TBookmark);
 var
   Rec: TMemoryRecord;
   SavePos: Integer;
   Accept: Boolean;
...
//==============================================================================================
procedure TssMemoryData.InternalSetToRecord(Buffer: PChar);
begin
  InternalGotoBookmark(@PMemBookmarkInfo(Buffer + FBookmarkOfs)^.BookmarkData);
end;

//==============================================================================================
procedure TssMemoryData.InternalFirst;
begin
  FRecordPos := -1;
end;

//==============================================================================================
procedure TssMemoryData.InternalLast;
begin
...
//==============================================================================================
procedure TssMemoryData.AssignMemoryRecord(Rec: TMemoryRecord; Buffer: PChar);
  var
    I: Integer;
begin
...
//==============================================================================================
procedure TssMemoryData.SetMemoryRecordData(Buffer: PChar; Pos: Integer);
  var
    Rec: TMemoryRecord;
begin
  if State = dsFilter then Error(SNotEditing);
...
//==============================================================================================
procedure TssMemoryData.SetAutoIncFields(Buffer: PChar);
  var
    I, Count: Integer;
    Data: PChar;
begin
  Count := 0;
  for I := 0 to FieldCount - 1 do
    if (Fields[I].FieldKind in fkStoredFields) and (Fields[I].DataType = ftAutoInc)
    then begin
...
//==============================================================================================
procedure TssMemoryData.InternalAddRecord(Buffer: Pointer; Append: Boolean);
  var
    RecPos: Integer;
    Rec: TMemoryRecord;
begin
...
//==============================================================================================
procedure TssMemoryData.InternalDelete;
  var
    Accept: Boolean;
begin
...
//==============================================================================================
procedure TssMemoryData.InternalPost;
  var
    RecPos: Integer;
begin
  if State = dsEdit 
  then SetMemoryRecordData(ActiveBuffer, FRecordPos)
  else begin
...
//==============================================================================================
procedure TssMemoryData.OpenCursor(InfoQuery: Boolean);
begin
...
//==============================================================================================
procedure TssMemoryData.InternalOpen;
begin
...
//==============================================================================================
procedure TssMemoryData.InternalClose;
begin
  ClearRecords;
  FAutoInc := 1;
  BindFields(False);
...
//==============================================================================================
procedure TssMemoryData.InternalHandleException;
begin
  Application.HandleException(Self);
end;

//==============================================================================================
procedure TssMemoryData.InternalInitFieldDefs;
begin
end;

//==============================================================================================
function TssMemoryData.IsCursorOpen: Boolean;
begin
  Result := FActive;
end;
...
//==============================================================================================
function TssMemoryData.GetRecordCount: Integer;
begin
  Result := FRecords.Count;
end;

//==============================================================================================
function TssMemoryData.GetRecNo: Integer;
begin
  CheckActive;
  UpdateCursorPos;
...
//==============================================================================================
procedure TssMemoryData.SetRecNo(Value: Integer);
begin
...
//==============================================================================================
function TssMemoryData.IsSequenced: Boolean;
begin
  Result := not Filtered;
end;

//==============================================================================================
function TssMemoryData.Locate(const KeyFields: string;
  const KeyValues: Variant; Options: TLocateOptions): Boolean;
begin
  DoBeforeScroll;
...
//==============================================================================================
procedure TssMemoryData.EmptyTable;
begin
  if Active then begin
    CheckBrowseMode;
    ClearRecords;
...
//==============================================================================================
procedure TssMemoryData.EmptyDataSet;
begin
  EmptyTable;
end;

//==============================================================================================
procedure TssMemoryData.CopyStructure(Source: TDataSet);
  ...
  begin
    for I := FieldDefs.Count - 1 downto 0 do begin
      if not (FieldDefs.Items[I].DataType in ftSupported)
        then FieldDefs.Items[I].Free
...
//==============================================================================================
function TssMemoryData.LoadFromDataSet(Source: TDataSet; RecordCount: Integer; Mode: TLoadMode): Integer;
  var
...
//==============================================================================================
function TssMemoryData.SaveToDataSet(Dest: TDataSet; RecordCount: Integer): Integer;
var
  MovedCount: Integer;
begin
  Result := 0;
...
//==============================================================================================
procedure TssMemoryData.SortOnFields(const FieldNames: string;
...
//==============================================================================================
procedure TssMemoryData.QuickSort(L, R: Integer; Compare: TCompareRecords);
 var
   I, J: Integer;
   P: TMemoryRecord;
begin
...
//==============================================================================================
function TssMemoryData.CompareRecords(Item1, Item2: TMemoryRecord): Integer;
var
  Data1, Data2: PChar;
  F: TField;
  I: Integer;
...
//==============================================================================================
function TssMemoryData.GetIsIndexField(Field: TField): Boolean;
begin
  if FIndexList <> nil
    then Result := FIndexList.IndexOf(Field) >= 0
    else Result := False;
end;

//==============================================================================================
procedure TssMemoryData.CreateIndexList(const FieldNames: string);
 var
   Pos: Integer;
   F: TField;
begin
...
//==============================================================================================
procedure TssMemoryData.FreeIndexList;
begin
  FIndexList.Free;
  FIndexList := nil;
end;
...
begin
  FMode := Mode;
  FField := Field;
  FDataSet := FField.DataSet as TssMemoryData;
...
begin
  ...
  if (Pos >= 0) and (Pos < FDataSet.RecordCount) then begin
    ...
    if Rec <> nil then Result := PMemBlobArray(Rec.FBlobs)[FField.Offset];
  end;
end;
...
begin
  ...
  if FOpened and FCached and (FMode <> bmRead) then begin
    ...
    if Length(Temp) < FPosition + Count then SetLength(Temp, FPosition + Count);
...
begin
  ...
  if FOpened then
    if FCached
      then Result := Length(FDataSet.GetBlobData(FField, FBuffer))
      else Result := Length(GetBlobFromRecord(FField))
end;
