...
uses Classes, Types, Menus, Controls, Graphics, ImgList, Messages, ExtCtrls;

type
  TssMenuStyle = (msStandard, msOwnerDraw, msBtnLowered,
    msBtnRaised);
  ...
  private
    ...
    procedure SetStyle(Value: TssMenuStyle);
  ...
  published
    ...
    property Style: TssMenuStyle read FStyle write SetStyle default msStandard;
...
uses Windows, Forms, ssVCLUtil, CommCtrl, ssStrUtil, SysUtils;
...
function BtnStyle(MenuStyle: TssMenuStyle): TBtnStyle;
begin
  case MenuStyle of
...
begin
  ...
  if Item.Caption = Separator then begin
    ...
    if Assigned(Glyph) and not Glyph.Empty then begin
      W := Glyph.Width;
      if (Glyph is Graphics.TBitmap) and (NumGlyphs in [2..5]) then
        W := W div NumGlyphs;
      H := Glyph.Height;
...
procedure DrawDisabledBitmap(Canvas: TCanvas; X, Y: Integer; Bitmap: Graphics.TBitmap;
  State: TMenuOwnerDrawState);
const
...
procedure DrawMenuBitmap(Canvas: TCanvas; X, Y: Integer; Bitmap: Graphics.TBitmap;
...
begin
  ...
  if Assigned(Item) then begin
    ...
    begin
      ...
      if Glyph is Graphics.TBitmap then begin
        if (NumGlyphs in [2..5]) then begin
          ...
          (Rect.Bottom + Rect.Top - Glyph.Height) div 2, Graphics.TBitmap(Glyph),
          DrawLowered, State);
        Inc(Left, W + GetMarginOffset);
      end
...
constructor TssPopupMenu.Create(AOwner: TComponent);
begin
...
procedure TssPopupMenu.DefaultDrawItem(Item: TMenuItem; Rect: TRect;
  State: TMenuOwnerDrawState);
var
  Graphic: TGraphic;
  BackColor: TColor;
...
procedure TssPopupMenu.DefaultDrawMargin(ARect: TRect; StartColor,
  EndColor: TColor);
var
  R: Integer;
begin
...
destructor TssPopupMenu.Destroy;
begin
...
procedure TssPopupMenu.DrawItem(Item: TMenuItem; Rect: TRect;
  State: TMenuOwnerDrawState);
var
  Graphic: TGraphic;
  BackColor: TColor;
...
procedure TssPopupMenu.DrawMargin(ARect: TRect);
begin
...
procedure TssPopupMenu.GetImageIndex(Item: TMenuItem;
...
procedure TssPopupMenu.GetItemParams(Item: TMenuItem;
  State: TMenuOwnerDrawState; AFont: TFont; var Color: TColor;
...
procedure TssPopupMenu.ImageListChange(Sender: TObject);
begin
  if Sender = FImages then RefreshMenu(IsOwnerDrawMenu);
end;

function TssPopupMenu.IsOwnerDrawMenu: Boolean;
begin
  Result := (FStyle <> msStandard)
     or (Assigned(FImages) and (FImages.Count > 0));
end;

procedure TssPopupMenu.Loaded;
begin
  inherited Loaded;
  if IsOwnerDrawMenu then RefreshMenu(True);
end;

procedure TssPopupMenu.MeasureItem(Item: TMenuItem; var Width,
  Height: Integer);
begin
  if Assigned(FOnMeasureItem) then FOnMeasureItem(Self, Item, Width, Height)
end;

procedure TssPopupMenu.Notification(AComponent: TComponent;
...
procedure TssPopupMenu.Popup(X, Y: Integer);
const
...
procedure TssPopupMenu.Refresh;
begin
  RefreshMenu(IsOwnerDrawMenu);
end;

procedure TssPopupMenu.RefreshMenu(AOwnerDraw: Boolean);
begin
  Self.OwnerDraw := AOwnerDraw and not (csDesigning in ComponentState);
end;

procedure TssPopupMenu.SetBiDiModeFromPopupControl;
var
  AControl: TControl;
begin
...
procedure TssPopupMenu.SetImages(Value: TImageList);
var
...
procedure TssPopupMenu.SetStyle(Value: TssMenuStyle);
begin
...
function TssPopupMenu.UseRightToLeftAlignment: Boolean;
var
  AControl: TControl;
begin
  Result := False;
...
procedure TssPopupMenu.WMDrawItem(var Message: TWMDrawItem);
var
  State: TMenuOwnerDrawState;
  SaveIndex: Integer;
  Item: TMenuItem;
...
procedure TssPopupMenu.WMMeasureItem(var Message: TWMMeasureItem);
var
  Item: TMenuItem;
  Graphic: TGraphic;
  BackColor: TColor;
...
procedure TssPopupMenu.WndMessage(Sender: TObject; var AMsg: TMessage;
  var Handled: Boolean);
begin
