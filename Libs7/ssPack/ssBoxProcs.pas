...
procedure BoxDragOver(List: TWinControl; Source: TObject; X, Y: Integer; State: TDragState; var Accept: Boolean; Sorted: Boolean);
    ...
    try
      ...
      while I < BoxItems(SrcList).Count do begin
        if BoxGetSelected(SrcList, I) then begin
          ...
          if (SrcList is TListBox) and (DstList is TListBox) then
          begin
            //TListBox(DstList).State[NewIndex] :=
            //TListBox(SrcList).State[I];
          end;
...
begin
  for I := 0 to BoxItems(SrcList).Count - 1 do begin
    ...
    if (SrcList is TListBox) and (DstList is TListBox) then begin
