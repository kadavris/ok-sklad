...
type
  ...
  TssWindowHook = class(TComponent)
  private
    FActive: Boolean;
...
{ TControlHook }
  ...
  private
    ...
    procedure NotifyHooks(Order: THookOrder; var Msg: TMessage; var Handled: Boolean);
  ...
  public
    ...
    procedure HookControl;
    procedure UnhookControl;
    procedure AddHook(AHook: TssWindowHook);
    procedure RemoveHook(AHook: TssWindowHook);
...
begin
  ...
  while FList.Count > 0 do RemoveHook(TssWindowHook(FList.Last));
...
procedure TControlHook.AddHook(AHook: TssWindowHook);
begin
...
procedure TControlHook.RemoveHook(AHook: TssWindowHook);
begin
...
procedure TControlHook.NotifyHooks(Order: THookOrder; var Msg: TMessage; var Handled: Boolean);
  var
    I: Integer;
begin
  if (FList.Count > 0) and Assigned(FControl)
    and not (FDestroying or (csDestroying in FControl.ComponentState))
  then
    for I := FList.Count - 1 downto 0 do begin
      try
        if Order = hoBeforeMsg 
        then TssWindowHook(FList[I]).DoBeforeMessage(Msg, Handled)
        else if Order = hoAfterMsg 
             then TssWindowHook(FList[I]).DoAfterMessage(Msg, Handled);
      except
...
constructor TssWindowHook.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FActive := True;
end;

destructor TssWindowHook.Destroy;
begin
  Active := False;
  WinControl := nil;
  inherited Destroy;
...
procedure TssWindowHook.SetActive(Value: Boolean);
begin
  if FActive <> Value then
    if Value then HookControl else UnhookControl;
end;

function TssWindowHook.GetHookHandle: HWnd;
begin
...
procedure TssWindowHook.HookControl;
begin
...
function TssWindowHook.DoUnhookControl: Pointer;
begin
  Result := FControlHook;
...
procedure TssWindowHook.UnhookControl;
begin
  DoUnhookControl;
  FActive := False;
end;

function TssWindowHook.NotIsForm: Boolean;
begin
  Result := (WinControl <> nil) and not (WinControl is TCustomForm);
end;

function TssWindowHook.IsForm: Boolean;
begin
  Result := (WinControl <> nil) and ((WinControl = Owner) and
    (Owner is TCustomForm));
end;

procedure TssWindowHook.ReadForm(Reader: TReader);
begin
  if Reader.ReadBoolean then
    if Owner is TCustomForm then WinControl := TWinControl(Owner);
end;

procedure TssWindowHook.WriteForm(Writer: TWriter);
begin
  Writer.WriteBoolean(IsForm);
end;

procedure TssWindowHook.DefineProperties(Filer: TFiler);
  function DoWrite: Boolean;
  begin
    if Assigned(Filer.Ancestor) then
      Result := IsForm <> TssWindowHook(Filer.Ancestor).IsForm
    else Result := IsForm;
  end;
...
function TssWindowHook.GetWinControl: TWinControl;
begin
  if Assigned(FControlHook) 
    then Result := TControlHook(FControlHook).WinControl
    else Result := FControl;
end;

procedure TssWindowHook.DoAfterMessage(var Msg: TMessage; var Handled: Boolean);
begin
  if Assigned(FAfterMessage) then FAfterMessage(Self, Msg, Handled);
end;

procedure TssWindowHook.DoBeforeMessage(var Msg: TMessage; var Handled: Boolean);
begin
  if Assigned(FBeforeMessage) then FBeforeMessage(Self, Msg, Handled);
end;

procedure TssWindowHook.Notification(AComponent: TComponent; Operation: TOperation);
begin
...
procedure TssWindowHook.SetWinControl(Value: TWinControl);
  var
    SaveActive: Boolean;
    Hook: TControlHook;
begin
  if Value <> WinControl then begin
    ...
    if Assigned(Hook) and (Hook.FList.Count = 0) and Assigned(HookList)
      then PostMessage(HookList.Handle, CM_DESTROYHOOK, 0, Longint(Hook));
...
begin
  ...
  if VirtualProtect(Table, PageSize, PAGE_EXECUTE_READWRITE, @SaveFlag)
    then
      try
        Table^ := NewAddress;
      finally
