...
type
  ...
  TTBItemOption = (tboDefault, tboDropdownArrow, tboImageAboveCaption,
    tboLongHintInMenuOnly, tboNoRotation, tboShowHint, tboToolbarStyle,
    tboToolbarSize);
...
begin
  ...
  { Add shortcut text to hint if item is based on an action. (Based on
    code from TControlActionLink.DoShowHint.) }
  if (Result <> '') and Assigned(ActionLink) and
     (ActionLink.Action is TCustomAction) then begin
    Actn := TCustomAction(ActionLink.Action);
    if Actn.DoHint(Result) and Application.HintShortCuts and
       (Actn.ShortCut <> scNone) then
...
const
  ...
  ChangeOptions = [tboDefault, tboDropdownArrow, tboImageAboveCaption,
    tboNoRotation, tboToolbarStyle, tboToolbarSize];
var
...
type
  ...
  TTempPosition = record
    BoundsRect: TRect;
    Show, OffEdge, LineSep, Clipped: Boolean;
  end;
...
var
  ...
  Ctl: TControl;
  ChangedBold: Boolean;
  Total, J, TotalVisibleItems: Integer;
  IsFirst: Boolean;
  Viewer: TTBItemViewer;
  ...
  try
    ...
    for I := 0 to FViewerCount-1 do begin
      ...
      with NewPositions[I] do begin
        ...
        PositionItem (I, NewPositions[I], FIsToolbar);
...
//    DrawCanvas.Font.Name := 'GEO AKADEMIURI';
//    DrawCanvas.Font.CharSet := 0;
