...
type
  ...
  TSocketForm = class(TForm)
    ...
    procedure ConnectionListColumnClick(Sender: TObject; Column: TListColumn);
    procedure ConnectionListCompare(Sender: TObject; Item1, Item2: TListItem; Data: Integer; var Compare: Integer);
    ...
    try
      ...
      begin
        for i := 0 to Sections.Count - 1 do
          if CompareText(Sections[i], csSettings) <> 0 then CreateItem(StrToInt(Sections[i]));
      end 
      else CreateItem(-1);
...
begin
  ...
  try // finally
    try // except
      ...
      try
        Reg.ReadSections(Sections);
        for i := 0 to Sections.Count - 1 do TRegistry(Reg).DeleteKey(Sections[i]);

      finally
        Sections.Free;
      end;

      for i := 0 to PortList.Items.Count - 1 do TSocketDispatcher(PortList.Items.Objects[i]).WriteSettings(Reg);
