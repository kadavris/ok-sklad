...
type
  ...
  TMetaProduct = class(TMetaClass)
    protected
      ...
      procedure setStatus(const EN: Integer = 0; msg: String = '');
  ...
  procedure MakeWebProductTree(prodTree, langTree: TapTree);
  ...
  // returns Product's Custom Attribute 'Attr' from ProductsTree/'CustAttr' branch
  function getProdCustAttr(const AID: Integer; const Attr: String): String;
...
uses
  SysUtils, DateUtils, StrUtils, prConst, udebug, fMessageDlg, ClientData;
 ...
 * 4 - node data fixed or added missing positions
 ...
 node/data diagram with cid numbers see in EditWebShop module
}
function FixGroupNode(node: TapTreeNode; FixData: boolean; const langTree: TapTree): Integer;
  var
    i, nType: Integer;
  ...
  begin
    data := Node.FindData(0, cid);
    if (data = nil) or (data.dataType <> apTreeDTInt) then begin
      try
        s := 'FixGroupNode/FixIntData: fixing: ';
        If data = nil
          then s := s + 'missing data cid: ' + IntToStr(cid)
          else s := s + 'bad data: ' + data.classTechDetails;

        debugInstantLog(s);
      except
        debugInstantLog('corrupted datalist: ' + node.classTechDetails);
      end;
  ...
  begin
    data := Node.FindData(0, cid);
    if (data = nil) or (data.dataType <> apTreeDTTxt) then begin
      try
        s := 'FixGroupNode/FixTxtata: fixing: ';
        If data = nil
          then s := s + 'missing data cid: ' + IntToStr(cid)
          else s := s + 'bad data: ' + data.classTechDetails;
  ...
  //-------------------------------------------------------------------------------------------------------
  // strips off data with invalid CIDs
  ...
  begin
    ...
    for i := 0 to node.Datacount - 1 do begin
      ...
      then begin
        try
          debugInstantLog('FixGroupNode/ChopData: removing: ' + node.Data[i].classTechDetails);
        except
        end;
        node.Data[i].Delete;
        Result := Result or 4;
  ...
  begin
    ...
    FixIntData(ln, 0, 1);             // mandatory creating/resetting node-type to lang,
    ln.FindData(0, 0).asInteger := 1; // even if it is found by fixintdata first
    ...
    FixIntData(ln, 7, 0); // custom <Alias> Off
...
begin
  ...
  nType := 0; // master-node by default, but we'll re-check it later and more precise

  if (node.Pos <> 1972) and (langTree.FindFirst(node.Name, False) <> nil)
    then nType := 1; // no master marker and in languages tree.
  ...
  then begin
    if ntype = 0 then begin  // checking for signs of master node
      if ((Node.FindData(0, 1) = nil) or (Node.FindData(0, 2) = nil))
          or ((Node.FindData(0, 1).dataType <> apTreeDTTxt)      // description text
              and (Node.FindData(0, 2).dataType <> apTreeDTInt)) // position index
      then begin
        debugInstantLog('FixGroupNode: node master (probably) data too corrupted: ' + node.classTechDetails);
      ...
      then begin
        debugInstantLog('FixGroupNode: node lang (probably) data too corrupted: ' + node.classTechDetails);
  ...
  // here is master node
  node.pos := 1972; //another master-type node marker
  ChopData(Node, 2);
  FixIntData(node, 0, 0); // just in case
  node.FindData(0, 0).asInteger := 0; // master-type
...
destructor TMetaProdNameMaps.Destroy;
  var
    ...
    i: integer;
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TMetaProdNameMaps.Free') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
//==============================================================================================
procedure TMetaProduct.setStatus(const EN: Integer = 0; msg: String = '');
begin
  FErrNo := EN;
  FStatusMessage := rs('MetaProduct', 'Status_' + IntToStr(FErrNo)) + ifThen(msg <> '', ': (' + msg + ')', '');
end;
...
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TMetaProduct.LoadData') else _udebug := nil;{$ENDIF}
  ...
  with ds do try
    ...
    {$IFDEF UDEBUG}_udebug.CheckPoint('opened');{$ENDIF}
    ...
    if FieldByName(FFieldNames.ExtName['DateAdded']).isNull
      ...
      else FDateAdded := FieldByName(FFieldNames.ExtName['DateAdded']).AsDateTime;

    if FieldByName(FFieldNames.ExtName['DateModified']).isNull
      then FdateModified := incDay(now, -1)
      else FDateModified := FieldByName(FFieldNames.ExtName['DateModified']).AsDateTime;

    FID := FieldByName(FFieldNames.ExtName['ID']).asInteger;

    {$IFDEF UDEBUG}_udebug.CheckPoint('trees');{$ENDIF}
    ...
    with newDataSet do try
      ...
      // getting remains at warehouses
      {$IFDEF UDEBUG}_udebug.CheckPoint('remains');{$ENDIF}
      remain := nil;
    ...
    on e: Exception do begin
      FID := -1;
      Result := False;
      setStatus(mpscLoadError, e.message);
      debuginstantlog('! ERROR: TmetaProduct.loaddata: except: ' + e.message {$IFDEF UDEBUG}+ ' Checkpoints: ' + _udebug.getCheckPoints{$ENDIF});
      raise;
    end;
  end;
...
function TMetaProduct.XMLForWeb(out stream: TStringStream; const WSID: Integer; const ProdID: Integer = 0): Boolean;
  var
    ...
    {$IFDEF UDEBUG}_udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TMetaProduct.XMLForWeb') else _udebug := nil;{$ENDIF}
...
begin
  ...
  if AnsiStartsText('ProdCustAttr', Attr)
    then s := AnsiMidStr(Attr, 14, 999) // strip class part (ProdCustAttr.someattr)
    else s := Attr;

  if ProductsTree = nil
    then ProductsTree := TapTree.Create(tree_Products, cfOwnerTypeProduct, AID)
    else ProductsTree.LoadData(AID);

  n := ProductsTree.findFirst(tree_ProdCustAttr, True);
  if n = nil then Exit; // no attributes defined

  n := n.FindFirst(s, False);
  if n = nil then begin
