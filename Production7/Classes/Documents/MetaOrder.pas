...
type
  ...
  TMetaOrder = class(TMetaClass) // should be metadoc, but it is still defunct
    ...
    public
      ...
      function LoadXML(var AXMLif: IXMLDocument; var Order: IXMLNode): Boolean; 
        ...
        if FItems.isItemsModified then begin // Запись позиций в накладную
          ...
          for i := 0 to FItems.Count - 1 do begin
            ...
            if FItems[i].PosType = WBMetaItemTangible then begin // product
              ...
              if FItems[i].Price.hasDiscounts
                then Params.ParamByName('DISCOUNT').AsFloat := FItems[i].Price.Discounts[0].Value
                else Params.ParamByName('DISCOUNT').AsFloat := 0.0;

              if FItems[i].Price.hasTaxes
                then Params.ParamByName('NDS').AsFloat := FItems[i].Price.Taxes[0].Value
                else Params.ParamByName('NDS').AsFloat := 0.0;
...
//==============================================================================================
procedure TMetaOrder.loadXMLcallback(const topNode, cbNode: IXMLNode);
begin
