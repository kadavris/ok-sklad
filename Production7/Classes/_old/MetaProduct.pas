...
//==============================================================================================
destructor TMetaProdNameMaps.Free;
  var
    {$IFDEF UDEBUG}_udebug: Tdebug;{$ENDIF}
    i: integer;
begin
...
begin
  FErrNo := EN;
  FStatusMessage := rs('MetaProduct', 'Status_' + IntToStr(FErrNo));
end;
...
begin
  ...
  if ProdID <> 0 then begin // requested to load specific product
    ...
    try
      ds := newDataSet;
      if not LoadData(ds, ProdID, True, False) then begin
        ds.Free;
        {$IFDEF UDEBUG}if _udebug <> nil then _udebug.Destroy;{$ENDIF}
        Exit;
      end;
  ...
  if not ExportXML(stream, True, True) then begin
    {$IFDEF UDEBUG}if _udebug <> nil then _udebug.Destroy;{$ENDIF}
    Exit;
  end;
