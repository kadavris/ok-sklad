...
type
  { actually for 'loaded' states there can be absolutely nothing,
    but succesful attempt to load data was made, which rendered no output
  }
  TapNodeLoadedState = (
    apNodeLoadedNone = 0, // for node that had not loaded any data yet
    apNodeLoadedFull = 1, // all data loaded for this node
    apNodeLoadedFullTree = 2, // all data loaded for this node and sub-tree (used to speed-up checks)
    apNodeLoadedPart = 3 // some but not all data loaded
  );
...
//----------------------------------------------------------------------------------
//----------------------------------------------------------------------------------
  // stores the state of loading data items for specific OwnerID. Simply to prevent DB overuse.
  TapDataLoadCache = class
    private
      FOID: array of Integer;
      FState: array of TapNodeLoadedState;
      FLoadedAll: Boolean; // shortcut for 'is all data loaded?'

      constructor Create;
      destructor Destroy; override;

      function  getState(const OID: Integer): TapNodeLoadedState;
      procedure setState(const OID: Integer; const AState: TapNodeLoadedState);

    public
      procedure Clear;

      property State[const OID: Integer]: TapNodeLoadedState read getState write setState; default;
  end;
...
//----------------------------------------------------------------------------------
  TapTreeNode = class(TMetaClassList) // for easy access to data fetched from CustomTree tables
    ...
    public
      ...
      function ChildrenLoadedState(includeMySelf: Boolean = False; const OID: integer = -1): TapNodeLoadedState;
      ...
      function Serialize(var SO: TSerializator): Boolean;
      function deSerialize(const SO: TSerializator; const AParent: TapTreeNode = nil): Boolean;
      ...
      function getLoadedState(const OID: Integer = -1): TapNodeLoadedState;
      ...
      property LoadedState[const AOID: Integer]: TapNodeLoadedState read getLoadedState;
...
//----------------------------------------------------------------------------------
  TapTree = class(TapTreeNode)
    ...
    public
      ...
      // export/import
      function Serialize(var SO: TSerializator): Boolean;
      function deSerialize(const SO: TSerializator): Boolean;
...
begin
  ...
  with Fparent.Tree.dataSet do begin
    ...
    try
      ...
      if FCID >= 0
        then Params.ParamByName('cid').AsInteger := FCID
        else Params.ParamByName('cid').Clear;
...
begin
  ...
  try // finally
    SO.beginsection('apTreenodeData');

    if not inherited Serialize(SO) then Exit; // do the base class first

    SO.PutInt('dtype', FdataType);
    SO.PutInt('cid', FCID);
    SO.PutInt('oid', FOwnerID);
...
begin
  ...
  try // finally
    ...
    FdataType := SO.Int; if SO.Error > 0 then Exit;
    if (FdataType < 0) or (FdataType > apTreeDTMax) then Exit;

    FCID := SO.Int; if SO.Error > 0 then Exit;
    FOwnerID := SO.Int; if SO.Error > 0 then Exit;

    case FdataType of
      apTreeDTInt: FValue := SO.Int;

      apTreeDTNum,
      apTreeDTDbl: FValue := SO.Ext;

      apTreeDTTxt: FValue := SO.Str;
    end; // case FdataType
...
//==============================================================================================
//==============================================================================================
procedure TapDataLoadCache.Clear;
begin
  setlength(FOID, 0);
  setlength(FState, 0);
  FLoadedAll := False;
...
//==============================================================================================
constructor TapDataLoadCache.Create;
begin
  Clear;
end;

//==============================================================================================
destructor TapDataLoadCache.Destroy;
begin
  Clear;
  inherited;
end;
...
//==============================================================================================
function  TapDataLoadCache.getState(const OID: Integer): TapNodeLoadedState;
  var
    i: Integer;
begin
  if FLoadedAll then begin
    Result := apNodeLoadedFull;
    Exit;
  end;

  if OID = -1 then begin
    Result := apNodeLoadedNone; // Floadedall checked above ;)
    Exit;
  end;
  
  Result := apNodeLoadedNone;

  for i := 0 to length(FOID) - 1
    do if FOID[i] = OID then begin
      Result := FState[i];
      Exit;
    end;
end;
...
//==============================================================================================
procedure TapDataLoadCache.setState(const OID: Integer; const AState: TapNodeLoadedState);
  var
    i, l: Integer;
begin
  if OID = -1 then begin
    Clear;
    FLoadedAll := (AState = apNodeLoadedFull);
    Exit;
  end;
  ...
  for i := 0 to l
    do if FOID[i] = OID then Break;

  if i > l then begin
    setlength(FOID, l + 2);
    setlength(FState, l + 2);
    i := l + 1;
  end;
...
begin
  ...
  if FDataCache = nil then FDataCache := TapDataLoadCache.Create;
  ClearData;
...
begin
  ...
  if FData <> nil then begin
    for i := 0 to FData.Count - 1 do
      ...
      then begin
        if not TapTreeNodeData(FData[i]).Fnew and (FDataCache[AOwnerID] = apNodeLoadedFull)
          then FDataCache[AOwnerID] := apNodeLoadedNone;
  ...
  if FData.Count = 0 then FDataCache.clear;
...
begin
  ...
  if FDeleted or FNew or (FDataCache[AOwnerID] = apNodeLoadedFull)
    then Exit; // save the bandwidth
  ...
  try // finally
    with ds do try
      ...
      if LoadChildren
        ...
        FDataCache[AOwnerID] := apNodeLoadedFull;
...
begin
  ...
  try
    ...
    for i := 0 to FItems.Count - 1 do begin
      ...
      if AnsiLowerCase(Result.Name) = AnsiLowerCase(AName) then Exit;

      if inChildren then begin
        Result := Result.FindFirst(AName, True);
...
begin
  ...
  try // finally
    ...
    for i := 0 to FItems.Count - 1 do begin
      ...
      if Result.NodeData = AData then Exit;
...
begin
  ...
  try //finally
    if VarisNull(AFind) and VarisClear(FFind) then Exit; // no previous findfirst
    ...
    if VarIsClear(FFind) then begin // FindFirst didn't get here
      ...
      if varInteger = VarType(AFind) then begin
        ...
        if AnsiLowerCase(FName) = AnsiLowerCase(VarToStr(AFind)) then Result := Self;
      end;
      Exit;
...
begin
  ...
  if not Fnew then setModified(True);
...
//==============================================================================================
function TapTreeNode.ChildrenLoadedState(includeMySelf: Boolean = False; const OID: integer = -1): TapNodeLoadedState;
  var
    i: Integer;
    {$IFDEF UDEBUG}_udebug: Tdebug;{$ENDIF}
begin
  ...
  if includeMySelf
    then Result := FDataCache[OID]
    else Result := apNodeLoadedFull; // assuming there is anything loaded down there by default.

  if Result = apNodeLoadedFullTree // skipping sub-nodes check as there is can be no 
  then begin
    Result := apNodeLoadedFull;
    Exit;
  end;
  ...
  for i := 0 to FItems.Count - 1 do begin
    ...
    case TapTreeNode(FItems[i]).ChildrenLoadedState(True, OID) of
...
//==============================================================================================
function TapTreeNode.getLoadedState(const OID: Integer = -1): TapNodeLoadedState;
begin
  Result := FDataCache[OID];
end;
...
begin
  ...
  try // finally
    SO.BeginSection('apTreeNode');

    if not inherited Serialize(SO) then Exit; // this will put a count of data nodes

    SO.PutInt('pid', FParent.ID);
    SO.PutInt('pos', FPos);
    SO.PutInt('ndata', FNodeData);
    SO.PutInt('dcount', FData.Count);

    SO.BeginSection('node_' + IntTostr(ID) + '_data_Nodes');
    for i := 0 to FData.Count - 1 do
    ...
    SO.BeginSection('node_' + IntTostr(ID) + '_sub_Nodes');
    for i := 0 to FItems.Count - 1 do
    ...
    SO.EndSection('apTreeNode'); // ends sub-nodes and our sections too
    Result := True;
...
//==============================================================================================
function TapTreeNode.deSerialize(const SO: TSerializator; const AParent: TapTreeNode = nil): Boolean;
  var
    s: String;
...
begin
  ...
  try // finally
    ...
    i := SO.Int; if SO.Error > 0 then Exit;
    if AParent.ID <> i then Exit;

    FPos := SO.Int; if SO.Error > 0 then Exit;
    FNodeData := SO.Int; if SO.Error > 0 then Exit;
    newData := SO.Int; if SO.Error > 0 then Exit; // additional/replacements for data items

    // MetaClassList deserializator set the expected count for FItems, so we need to fill all these
    // better to read data as is, till the end of section
    s := 'node_' + IntToStr(ID);
    while True do begin
      if (SerStateSectionEnd in SO.State) and (SO.SectionName = s + '_data_Nodes') then Break;
...
begin
  ...
  for i := 0 to Fitems.Count - 1 do TapTreeNode(FItems[i]).FDataCache.Clear;
...
begin
  ...
  try // finally
    if ChildrenLoadedState(True, AOwnerID) = apNodeLoadedFull then Exit; // saving bandwidth
    ...
    with FDS do try
      ...
      for i := 1 to 4 do begin // we cycle thru datatypes and load any of them sequentially
        ...
        Open;
        while not eof do begin
          n := Self.NodeByID(FieldByName('CTNodeID').AsInteger); // finding parent node for this data item

          if n <> nil then begin
            n.FDataCache[AOwnerID] := apNodeLoadedFull;
      ...
      FDataCache[AOwnerID] := apNodeLoadedFullTree;
