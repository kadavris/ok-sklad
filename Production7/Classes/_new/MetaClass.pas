...
type
  TMetaClass = class
    ...
    protected
      ...
      function  loadXMLNode(var topNode, Node: IXMLNode; paramIndex: Integer = -1): Boolean; virtual;
      ...
      procedure setID(const Value: Integer); virtual;// ID > 0 sets FNew = False
      procedure setName(const Value: String); virtual;
      procedure setDate(const Value: TDateTime); virtual;

      function  getProp(const Index: String): Variant; virtual;
      procedure setProp(const Index: String; const Value: Variant); virtual;
      function  getStreamProp(const Index: String): TMemoryStream; virtual;
      procedure setStreamProp(const Index: String; const Value: TMemoryStream); virtual;

    public
      constructor Create(const AParent: TMetaClass); virtual;
      ...
      procedure setError(ACode: Integer; AText: String = ''; ADetails: String = ''; Append: Boolean = False); virtual;
      function lastErrorText: String; virtual;

      // it collects error messages from all of it's sub-classes, mainly for debugging and human awareness
      function reportProblems(clearErrors: Boolean = True): String; virtual;
      ...
      function Serialize(const SO: TSerializator): Boolean; virtual;// when called from descendant class
      function deSerialize(const SO: TSerializator; const AMetaParent: TMetaClass): Boolean; virtual;
      function LoadXML(var Node: IXMLNode): Boolean; virtual;
      // for use in child's loadXML when there is unknown prop encountered and the parent might be able to process it
      procedure loadXMLCallBack(topNode, cbNode: IXMLNode); virtual;
  ...
  TMetaClassList = class(TMetaClass)
    protected
      ...
      function  Add(const Value: TMetaClass): Integer; virtual;
      procedure loadXMLcallback(const topNode, cbNode: IXMLNode); virtual;
      function  LoadXMLNode(var topNode, Node: IXMLNode; paramIndex: Integer = -1): Boolean; virtual;

      // property processing
      function  getItem(const idx: Integer): TMetaClass; virtual;
      procedure setItem(const idx: Integer; const Value: TMetaClass); virtual;
      function  getCount: Integer; virtual;
      function  getItemsModified: Boolean; virtual;
    ...
    public
      ...
      function Serialize(const SO: TSerializator): Boolean; virtual; // when called from desvendant class
      function deSerialize(const SO: TSerializator; const AMetaParent: TMetaClass): Boolean; virtual;
...
begin
  ...
  Clear; // beware! Fucking borland ALWAYS calls topmost heir's methods...
...
begin
  ...
  if FItems.Count > 0 then begin
    for i := 0 to FItems.Count - 1 do
      try
        TObject(FItems[i]).Free; // this will call topmost Destructor automatically
...
begin
  ...
  for i := 0 to FItems.Count - 1 do
    if FItems[i] <> nil then Result := TMetaClass(FItems[i]).isModified or Result;
end;
...
function TMetaClassList.deSerialize(const SO: TSerializator; const AMetaParent: TMetaClass): Boolean;
  var
    p: Integer;
begin
  Result := False;
