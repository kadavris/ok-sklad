...
//----------------------------------------------------------------------------------
  TapTreeNodeData = class(TMetaClass)
    ...
    public
      constructor Create(AParent: TapTreeNode; const ds: TssClientDataSet = nil; const AOwnerID: Integer = -1); reintroduce;
      destructor Destroy; override;
      ...
      function Serialize(const SO: TSerializator): Boolean; override;
      function deSerialize(const SO: TSerializator; const AParent: TapTreeNode): Boolean; reintroduce;
...
//----------------------------------------------------------------------------------
  TapTreeNode = class(TMetaClassList) // for easy access to data fetched from CustomTree tables
    ...
    public
      constructor Create(const AName: String; const ATree: TapTree); reintroduce; overload; // for empty node (special trick)
      constructor Create(const AName: String; const AParent: TapTreeNode); reintroduce; overload; // for empty node too. standard
      constructor Create(const ATree: TapTree); reintroduce; overload; // standard way of node creation
      destructor  Destroy; override;
      ...
      procedure setModified(const Value: Boolean); override; // mainly to force node saving to DB
      ...
      function Serialize(const SO: TSerializator): Boolean; virtual;
      function deSerialize(const SO: TSerializator; const AParent: TapTreeNode): Boolean; virtual;
...
//----------------------------------------------------------------------------------
  TapTree = class(TapTreeNode)
    ...
    public
      ...
      // export/import
      function Serialize(const SO: TSerializator): Boolean; overload; override;
      function Serialize(var Stream: TStream): Boolean; reintroduce; overload;
...
//----------------------------------------------------------------------------------
  TapWoods = class(TMetaClassList) // tree list class
    ...
    public
      ...
      function  Add(const Aname: String; const AOwnerType: Integer; const LoadOwnerID: Integer = apTreeLoadOIDNone): TapTree; reintroduce; {overload;} // add/load tree by name
...
//==============================================================================================
function TapTreeNodeData.Serialize(const SO: TSerializator): Boolean;
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
...
begin
  ...
  FData := TList.Create;
  FRTData := TSimpleAssociativeArray.Create;
...
//==============================================================================================
procedure TapTreeNode.setModified(const Value: Boolean);
  //{$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
...
//==============================================================================================
function TapTreeNode.Serialize(const SO: TSerializator): Boolean;
  var
    i: Integer;
    {$IFDEF UDEBUG}_udebug: Tdebug;{$ENDIF}
begin
...
begin
  ...
  try // finally
    newItems := Count;
    if not inherited deSerialize(SO, nil) then Exit; // returns count of new items
    newItems := Count - newItems;
...
//==============================================================================================
function TapTree.Serialize(const SO: TSerializator): Boolean;
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TapTree.Serialize(SO)') else _udebug := nil;{$ENDIF}
