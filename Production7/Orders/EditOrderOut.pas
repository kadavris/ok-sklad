{$I ok_sklad.inc}
unit EditOrderOut; // client order
...
uses
  ...
  cxTreeView, ComCtrls, cxListView,  cxDBEdit, cxGraphics, prTypes,
  ...
  TB2Item, ssDBGrid, ssBaseWBDocDlg, ssBaseWBDocDlgWithPayment,
  ssPageControl, xCtrls, okSearchEdit, okClasses, xLngDefs;

type
  TfrmEditOrderOut = class(TfrmBaseWBDocDlgWithPayment)
...
var
  frmEditOrderOut: TfrmEditOrderOut;
...
//==============================================================================================
function TfrmEditOrderOut.CheckDatesIn: boolean;
  var
    eStr, eStr2, m, tmpstr :String;
    i:integer; tmpMaxDate:TDateTime;
    {$IFDEF UDEBUG}_udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.CheckDatesIn') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.SetAttDate(const Value: TDate);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.SetAttDate') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.SetAttNum(const Value: string);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.SetAttNum') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.GetSummAll;
 var
   i, j: Integer;
   BM: TBookmark;
   ...
   {$IFDEF UDEBUG}_udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.GetSummAll') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.WMRefresh(var M: TMessage);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.WMRefresh') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.setid(const Value: integer);
 var
   ...
   {$IFDEF UDEBUG}_udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelLow, DEBUG_unit_ID, 'TfrmEditOrderOut.setid') else _udebug := nil;{$ENDIF}
  ...
  with newDataSet do
    try
      ...
      if ((Value = 0) or IsPattern) and OrdersOutAutoNum then begin
        APreffix := OrdersOutPrefix;
        ASuffix := OrdersOutSuffix;
        FLastGen := '';

        FCurrNum := GetDocNumEx(dmData.SConnection, dtOrderOut, 1, APreffix, ASuffix,
      ...
      if Value <> 0 then begin
        Self.Caption := rs('fmWaybill', 'TitleEditOrderOut');
        ...
        Self.Caption := rs('fmWaybill','TitleAddAccOut');
        mdDet.Open;
        mdWMatTurn.Open;
        mdWMatTurnOld.CopyFromDataSet(mdWMatTurn);
        PayDocChecked := False;
...
//==============================================================================================
procedure TfrmEditOrderOut.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
 var
   NewRecord: boolean;
   BM: TBookmark;
   FPosID: Integer;
   {$IFDEF UDEBUG}_udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.FormCloseQuery') else _udebug := nil;{$ENDIF}
  ...
  try // finally
    if ModalResult in [mrCancel] then begin
      if FLastGen = '' then begin
        if GetDocNum(dmData.SConnection, dtOrderOut, 0) = FCurrNum
          then GetDocNum(dmData.SConnection, dtOrderOut, -1);
      end
      else begin
    ...
    if True{CheckDatesIn} then begin
      ...
      with newDataSet do try // finally
        try //except
          TrStart;
          ...
          while not mdWMatTurn.Eof do begin
            ...
            Params.ParamByName('TurnType').AsInteger := wbtOrderOut;//2; - left from accout
          ...
          if FPayDocModified then begin//Если платёж изменён, то записываем его
            ...
            if chbPay.Checked then begin
              ...
              Params.ParamByName('reason').AsString := RS('fmWaybill','DocOrderOut') + amountPrefix + edNum1.Text;
          ...
          on e: exception do begin
            debugInstantLog('! ERROR: TfrmEditOrderOut.FormCloseQuery: except: ' + e.Message);
            TrRollback;
            raise;
          end;
        end;
...
//==============================================================================================
procedure TfrmEditOrderOut.ActionListUpdate(Action: TBasicAction; var Handled: Boolean);
 var
  FEnToDate: Boolean;
  FEnPay: Boolean;
  {$IFDEF UDEBUG}_udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.ActionListUpdate') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.DataModified(Sender: TObject);
  //{$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  //if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.DataModified') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.SetCaptions;
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.SetCaptions') else _udebug := nil;{$ENDIF}

  inherited;

  with LangMan do begin
    panTitleBar.Caption := rs('fmWaybill', 'DocOrderOut');
    colPType.Caption := '';
    edKAgent.InitRes;
    ...
    chbToDate.Properties.Caption := GetRS('fmWaybill','ValidToDate') + ':';
...
//==============================================================================================
procedure TfrmEditOrderOut.SetParentName(const Value: string);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  //if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.SetParentName') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.aCInsExecute(Sender: TObject);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.aCInsExecute') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.aCDelExecute(Sender: TObject);
 var
   FItem: TListItem;
   i: Integer;
   {$IFDEF UDEBUG}_udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.aCDelExecute') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.edNum1PropertiesChange(Sender: TObject);
  //{$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  //if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.edNum1PropertiesChange') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.chbPostingPropertiesChange(Sender: TObject);
  //{$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  //if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.chbPostingPropertiesChange') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.dbgWaybillDetCustomDrawCell(Sender: TObject; ACanvas: TCanvas; ARect: TRect; ANode: TdxTreeListNode; AColumn: TdxTreeListColumn; ASelected, AFocused, ANewItemRow: Boolean; var AText: String; var AColor: TColor; AFont: TFont; var AAlignment: TAlignment; var ADone: Boolean);
  //{$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  //if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.dbgWaybillDetCustomDrawCell') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.mdMatsAfterPost(DataSet: TDataSet);
  //{$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  //if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.mdMatsAfterPost') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.mdMatsAfterDelete(DataSet: TDataSet);
  //{$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  //if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.mdMatsAfterDelete') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.mdMatsAfterEdit(DataSet: TDataSet);
  //{$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  //if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.mdMatsAfterEdit') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.FormShow(Sender: TObject);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.FormShow') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.aCUpdExecute(Sender: TObject);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.aCUpdExecute') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.FormClose(Sender: TObject; var Action: TCloseAction);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.FormClose') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.mdDetAfterPost(DataSet: TDataSet);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.mdDetAfterPost') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.mdDetAfterDelete(DataSet: TDataSet);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.mdDetAfterDelete') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.aSetTimeExecute(Sender: TObject);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.aSetTimeExecute') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.aAddKAExecute(Sender: TObject);
  var
    aid: integer;
    {$IFDEF UDEBUG}_udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.aAddKAExecute') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.aShowRealPriceExecute(Sender: TObject);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.aShowRealPriceExecute') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.colSummDefGetText(Sender: TObject; ANode: TdxTreeListNode; var AText: String);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.colSummDefGetText') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.dbgWaybillDetDblClick(Sender: TObject);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.dbgWaybillDetDblClick') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.FormResize(Sender: TObject);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.FormResize') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.chbPayPropertiesChange(Sender: TObject);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.chbPayPropertiesChange') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.cbPayTypePropertiesChange(Sender: TObject);
  //{$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  //if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.cbPayTypePropertiesChange') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.edPaySummPropertiesChange(Sender: TObject);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.edPaySummPropertiesChange') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.cbCurrPropertiesChange(Sender: TObject);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.cbCurrPropertiesChange') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.cbCurrPropertiesEditValueChanged(Sender: TObject);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.cbCurrPropertiesEditValueChanged') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.WMDelete(var M: TMessage);
  //{$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  (*{$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.WMDelete') else _udebug := nil;{$ENDIF}
  {$IFDEF UDEBUG}if _udebug <> nil then _udebug.Destroy;{$ENDIF}
  *)
end;

//==============================================================================================
procedure TfrmEditOrderOut.lcbPersonNameKeyPress(Sender: TObject; var Key: Char);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.lcbPersonNameKeyPress') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.dbgWaybillDetKeyPress(Sender: TObject; var Key: Char);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.dbgWaybillDetKeyPress') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.lcbPersonNamePropertiesChange(Sender: TObject);
  //{$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  //if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.lcbPersonNamePropertiesChange') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
function TfrmEditOrderOut.KAName(kaid:integer):string;
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.KAName') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.chbToDatePropertiesChange(Sender: TObject);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.chbToDatePropertiesChange') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.aRemoveRsvExecute(Sender: TObject);
  var
    i: Integer;
    {$IFDEF UDEBUG}_udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.aRemoveRsvExecute') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.aMatInfoExecute(Sender: TObject);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.aMatInfoExecute') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.colStatusGetText(Sender: TObject; ANode: TdxTreeListNode; var AText: String);
  //{$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  //if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.colStatusGetText') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.DelPos;
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.DelPos') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.WMLayoutChanged(var M: TMessage);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.WMLayoutChanged') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.FormCreate(Sender: TObject);
  var
    Temp: Integer;
    {$IFDEF UDEBUG}_udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.FormCreate') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.aSelectAllExecute(Sender: TObject);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.aSelectAllExecute') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.aSetRsvExecute(Sender: TObject);
  var
    ...
    {$IFDEF UDEBUG}_udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.aSetRsvExecute') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.aBallanceExecute(Sender: TObject);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.aBallanceExecute') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.FormDestroy(Sender: TObject);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.FormDestroy') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.edKAgentEnter(Sender: TObject);
  //{$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  //if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.edKAgentEnter') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.edKAgentExit(Sender: TObject);
  //{$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  //if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.edKAgentExit') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.btnPayMPersonMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
  //{$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  //if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.btnPayMPersonMouseDown') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.btnPayMPersonMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
  //{$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  //if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.btnPayMPersonMouseUp') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.aPrintExecute(Sender: TObject);
 var
   ...
   {$IFDEF UDEBUG}_udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.aPrintExecute') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.frDocGetValue(const ParName: String; var ParValue: Variant);
  var
    FSumm, FWONDS, FNDS: Extended;
    {$IFDEF UDEBUG}_udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.frDocGetValue') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.lcbPayMPersonPropertiesChange(Sender: TObject);
  //{$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  //if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.lcbPayMPersonPropertiesChange') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.frDocEndDoc;
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.frDocEndDoc') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.aAddSvcExecute(Sender: TObject);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.aAddSvcExecute') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.aAddMatExecute(Sender: TObject);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.aAddMatExecute') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.aAddMatListExecute(Sender: TObject);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.aAddMatListExecute') else _udebug := nil;{$ENDIF}
  ...
  ShowModalWH(Self, MaxDateTime, 0, 7, 0, Integer(@DocInfo));

  {$IFDEF UDEBUG}if _udebug <> nil then _udebug.Destroy;{$ENDIF}
end;

//==============================================================================================
procedure TfrmEditOrderOut.aSvcInfoExecute(Sender: TObject);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.aSvcInfoExecute') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.SetSvcPresent(const Value: Boolean);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.SetSvcPresent') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.colNormGetText(Sender: TObject; ANode: TdxTreeListNode; var AText: String);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.colNormGetText') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.CheckSvc;
 var
   i: Integer;
   FSvc: Boolean;
   {$IFDEF UDEBUG}_udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.CheckSvc') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.ChangeMatsCur;
  var
    BM:TBookmark;
    {$IFDEF UDEBUG}_udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.ChangeMatsCur') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.aShowSNExecute(Sender: TObject);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.aShowSNExecute') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.aSetRsvAllExecute(Sender: TObject);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.aSetRsvAllExecute') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.aRemoveRsvAllExecute(Sender: TObject);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.aRemoveRsvAllExecute') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.dbgWaybillDetGetOptionsTitle(Sender: TObject; var AText: String; var ImgList: TImageList; var AIndex: Integer);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.dbgWaybillDetGetOptionsTitle') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.colAmountGetText(Sender: TObject; ANode: TdxTreeListNode; var AText: String);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.colAmountGetText') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.AddMats(var IDs: string);
  var
    ...
    {$IFDEF UDEBUG}_udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.AddMats') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.SaveAddProps(APosID: Integer);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.SaveAddProps') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.GetTaxes;
  var
    ACol: TdxDBTreeListColumn;
    FField: TFloatField;
    {$IFDEF UDEBUG}_udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.GetTaxes') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.colTaxGetText(Sender: TObject; ANode: TdxTreeListNode; var AText: String);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.colTaxGetText') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.colTaxRateGetText(Sender: TObject; ANode: TdxTreeListNode; var AText: String);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.colTaxRateGetText') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.SaveTaxes(APosID: Integer);
  var
    i: Integer;
    {$IFDEF UDEBUG}_udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.SaveTaxes') else _udebug := nil;{$ENDIF}
  ...
  try
    ...
    for i := 0 to mdDet.FieldCount - 1 do begin
      ...
      then begin
        Params.ParamByName('taxid').AsInteger :=
             StrToInt(Copy(mdDet.Fields[i].FieldName, 4, Length(mdDet.Fields[i].FieldName) - 3));
...
//==============================================================================================
procedure TfrmEditOrderOut.frDocBeginDoc;
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.frDocBeginDoc') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.WMSetText(var M: TMessage);
  var
    S: string;
    {$IFDEF UDEBUG}_udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.WMSetText') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.srcEntDataChange(Sender: TObject; Field: TField);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.srcEntDataChange') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.lEntExItemClick(Sender: TObject; Index: Integer);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.lEntExItemClick') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.aChangeEntExecute(Sender: TObject);
  var
    APreffix, ASuffix: string;
    {$IFDEF UDEBUG}_udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.aChangeEntExecute') else _udebug := nil;{$ENDIF}
  ...
  if cdsEnts.RecordCount > 1 then begin
    ...
    if (FID = 0) and OrdersOutAutoNum then begin
      if FLastGen <> '' then begin
        if Gen_ID(dmData.SConnection, FLastGen, 0) = FCurrNum
        then Gen_ID(dmData.SConnection, FLastGen, -1);
      end
      else if GetDocNum(dmData.SConnection, dtOrderOut, 0) = FCurrNum
           then GetDocNum(dmData.SConnection, dtOrderOut, -1);
...
//==============================================================================================
procedure TfrmEditOrderOut.edKAgentChange(Sender: TObject);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.edKAgentChange') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.edContrChange(Sender: TObject);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.edContrChange') else _udebug := nil;{$ENDIF}


  if (FCurrCtrl <> edKAgent) and (edContr.DocID > 0) and Visible
    then edKAgent.KAID := edContr.Obj.KAID;

  if (edContr.Obj.DocID > 0) and (cbCurr.KeyValue <> edContr.Obj.CurrID)
    then cbCurr.KeyValue := edContr.Obj.CurrID;

  if (edContr.Obj.DocID > 0) and (cbPayCurr.KeyValue <> edContr.Obj.CurrID)
    then cbPayCurr.KeyValue := edContr.Obj.CurrID;
...
//==============================================================================================
procedure TfrmEditOrderOut.aChangeEntAccExecute(Sender: TObject);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.aChangeEntAccExecute') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.lEntExGetHintText(Sender: TObject; Index: Integer; var AText: String);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.lEntExGetHintText') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.kaEntAccountChange(Sender: TObject);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.kaEntAccountChange') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.cdsEntAccBeforeOpen(DataSet: TDataSet);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.cdsEntAccBeforeOpen') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
function TfrmEditOrderOut.DoSetRsv(APosID, AMatID, AWID: Integer; AAmount: Extended): Boolean;
 var
   ...
   {$IFDEF UDEBUG}_udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.DoSetRsv') else _udebug := nil;{$ENDIF}
      ...
      try
        ...
        if not IsEmpty then begin
          ...
          try
            ...
            mdTmpIds.FieldByName('tmpid').AsInteger := dmData.SConnection.AppServer.Rsv_Add(FPosId,Fields[0].AsInteger,
                                        Fields[1].AsInteger,0, -FAmount);
      ...
      while not Eof and (tmpremain > 0) do begin
        if strtofloat(fieldbyname('amount').AsString) > 0 then begin
          ...
          then begin
            ...
            mdRsv.Post;
          end//if tmpremain <= strtofloat(fieldbyname('amount').AsString
          else begin
            mdRsv.Append;
            mdRsv.FieldByName('posid').AsInteger := FieldByName('posid').AsInteger;
            mdRsv.FieldByName('pricein').AsFloat := strtofloat(FieldByName('price').AsString);
        ...
        end; // if strtofloat(fieldbyname('amount').AsString) > 0 then begin
  ...
  if FRemain >= AAmount then begin
    ...
    while not mdRsv.Eof do begin
      ...
      then begin
        ...
        then begin
            ...
            TmpAmount := 0;
              while not Eof do begin
                if (FieldByName('posid').AsInteger=mdRsv.fieldbyname('posid').AsInteger)
                  then TmpAmount :=
                    RoundToA(TmpAmount +
                       RoundToA(strtofloat(FieldByName('amount').AsString), MatDisplayDigits), MatDisplayDigits);
          ...
          if TmpAmount < 0 then begin
            ...
            then begin
              ...
              if RoundToA(strtofloat(mdRsv.fieldbyname('amount').AsString)-strtofloat(mdWMatTurnOld.fieldbyname('amount').AsString)-(TmpAmount*-1), MatDisplayDigits)<>0 then begin
                mdTmpIds.Append;
                mdTmpIds.FieldByName('sourceid').AsInteger := APosID;
                mdTmpIds.FieldByName('posid').AsInteger := mdRsv.fieldbyname('posid').AsInteger;
          ...
          if RoundToA(strtofloat(mdRsv.fieldbyname('amount').AsString)
              - strtofloat(mdWMatTurnOld.fieldbyname('amount').AsString), MatDisplayDigits) <> 0
          then begin
            mdTmpIds.Append;
            mdTmpIds.FieldByName('sourceid').AsInteger := APosID;
            mdTmpIds.FieldByName('posid').AsInteger := mdRsv.fieldbyname('posid').AsInteger;
        ...
        if TmpAmount < 0 then begin
          if RoundToA(TmpAmount * -1, MatDisplayDigits) > RoundToA(strtofloat(mdRsv.fieldbyname('amount').AsString), MatDisplayDigits)
          then begin
            mdTmpIds.Append;
            mdTmpIds.FieldByName('sourceid').AsInteger := APosID;
            mdTmpIds.FieldByName('posid').AsInteger := mdRsv.fieldbyname('posid').AsInteger;
            ...
            if RoundToA(strtofloat(mdRsv.fieldbyname('amount').AsString) - (TmpAmount * -1), MatDisplayDigits) <> 0
            then begin
              mdTmpIds.Append;
              mdTmpIds.FieldByName('sourceid').AsInteger := APosID;
              mdTmpIds.FieldByName('posid').AsInteger := mdRsv.fieldbyname('posid').AsInteger;
    ...
    with mdWMatTurnOld do begin
      ...
      while not Eof do begin
        if (FieldByName('sourceid').AsInteger = APosID) and (FieldByName('flag').AsInteger <> 1) then begin
          ...
          mdTmpIds.FieldByName('amount').AsFloat := RoundToA(strtofloat(fieldbyname('amount').AsString), MatDisplayDigits)*-1;
...
//==============================================================================================
function TfrmEditOrderOut.DoRemoveRsv(APosID: Integer): Boolean;
  var
    BM: TBookmark;
    {$IFDEF UDEBUG}_udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.DoRemoveRsv') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.mdWMatTurnBeforeDelete(DataSet: TDataSet);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.mdWMatTurnBeforeDelete') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.mdWMatTurnAfterDelete(DataSet: TDataSet);
  //{$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  (*{$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.mdWMatTurnAfterDelete') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.FillMatsFromWMat(DS: TDataSet);
  var
    FPosID: Integer;
    {$IFDEF UDEBUG}_udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.FillMatsFromWMat') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.aSetDiscountExecute(Sender: TObject);
 var
   BM: TBookmark;
   i: Integer;
   {$IFDEF UDEBUG}_udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.aSetDiscountExecute') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.aOrdersInfoExecute(Sender: TObject);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.aOrdersInfoExecute') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.lcbPersonNameEnter(Sender: TObject);
  //{$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  //if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.lcbPersonNameEnter') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.lcbPersonNameExit(Sender: TObject);
  //{$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  //if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.lcbPersonNameExit') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.btnPersonNameMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.btnPersonNameMouseDown') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.btnPersonNameMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
  //{$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  //if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.btnPersonNameMouseUp') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.aActivateFindExecute(Sender: TObject);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.aActivateFindExecute') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
function TfrmEditOrderOut.CheckModified: Boolean;
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.CheckModified') else _udebug := nil;{$ENDIF}

  Result := False;

  if EditingDisabled then Exit;

  if (FModified or FPosModified)
     and (ssMessageDlg(rs('Common', 'DataChangedSave'), ssmtWarning, [ssmbYes, ssmbNo]) = mrYes)
  then Result := True;
...
//==============================================================================================
procedure TfrmEditOrderOut.UpdatePos;
 var
   ...
   {$IFDEF UDEBUG}_udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.UpdatePos') else _udebug := nil;{$ENDIF}
...
//==============================================================================================
procedure TfrmEditOrderOut.aAddSvcListExecute(Sender: TObject);
  {$IFDEF UDEBUG}var _udebug: Tdebug;{$ENDIF}
begin
  {$IFDEF UDEBUG}if Debugging then _udebug := Tdebug.Create(debugLevelHigh, DEBUG_unit_ID, 'TfrmEditOrderOut.aAddSvcListExecute') else _udebug := nil;{$ENDIF}
...
initialization
  {$IFDEF UDEBUG}
  Debugging := False;
  DEBUG_unit_ID := debugRegisterUnit('EditOrderOut', @Debugging, DEBUG_group_ID);
  {$ENDIF}
